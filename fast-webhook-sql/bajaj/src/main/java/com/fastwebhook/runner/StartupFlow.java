...
public class StartupFlow implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    ...
    log.info("Got webhook: {}", gen.getWebhook());
    log.info("Got accessToken: {}{}", (healthrx.isUseBearerPrefix() ? "Bearer ":""), "***");

    // 2) Compute which SQL to use
    String finalSql = pickSql(candidate.getRegNo());
    log.info("Chosen SQL (first 120 chars): {}", finalSql.length() > 120 ? finalSql.substring(0,120) + "…" : finalSql);

    // 3) Submit answer to provided webhook URL (fallback to fixed path if missing)
    String submitUrl = gen.getWebhook();
    boolean absolute = true;
    if (submitUrl == null || submitUrl.isBlank()) {
      submitUrl = healthrx.getFallbackSubmitPath();
      absolute = false; // will resolve against baseUrl
    ...
    WebClient.RequestBodySpec req = absolute
        ? submitClient.post().uri(submitUrl)
        : submitClient.post().uri(submitUrl);
  ...
  private String pickSql(String regNo) {
    ...
    if (digits.length() < 2) {
      // If only one digit is present, treat it as the last-two-digits anyway
